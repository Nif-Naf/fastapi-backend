...
run-only-unit-tests:
	source .venv/bin/activate && pytest	-v	--tb=short	--config-file=fastapi_backend/core/configs/pytest.ini tests/unit

# Migrations.
make_migration:
	source .venv/bin/activate && alembic --config=fastapi_backend/core/configs/alembic.ini revision
	--autogenerate